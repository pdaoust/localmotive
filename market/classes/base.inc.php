...
function isMarketObject ($obj) {
	return in_array(getObjectType($obj), array('Person', 'Item', 'OrderItem', 'Order', 'Price', 'Route', 'RouteDay', 'DeliveryDay', 'JournalEntry', 'Address'));
}

function isRecord ($obj) {
	return getObjectType($obj) == 'Record';
}

function isPerson ($obj) {
	return getObjectType($obj) == 'Person';
}

function isItem ($obj) {
	return getObjectType($obj, false, true) == 'Item';
}

function isOrderItem ($obj) {
	return getObjectType($obj) == 'OrderItem';
}

function isOrder ($obj) {
	return getObjectType($obj) == 'Order';
}

function isPrice ($obj) {
	return getObjectType($obj) == 'Price';
}

function isRoute ($obj) {
	return getObjectType($obj) == 'Route';
}

function isRouteDay ($obj) {
	return getObjectType($obj) == 'RouteDay';
}

function isDeliveryDay ($obj) {
	return getObjectType($obj, false, true) == 'DeliveryDay';
}

function getObjectType ($obj, $lowercase = false, $collapseInheritance = false) {
	if (is_object($obj)) {
		$objectType = get_class($obj);
		if ($collapseInheritance = true) {
			if ($objectType == 'OrderItem') {
				$objectType = 'Item';
			} else if ($objectType == 'RouteDay') {
				$objectType = 'DeliveryDay';
			}
		}
		if ($lowercase) {
			$objectType = strtolower(substr($objectType, 0, 1)).substr($objectType, 1);
		}
		return $objectType;
	}
	return null;
...
function getWaitingOrders ($dateToDeliver, $completed = false) {
	...
	if (!$db->query('SELECT * FROM orders WHERE orderType & ' . O_OLD . ' = ' . O_SALE . ($completed ? ' AND dateCompleted IS NOT NULL' : null) . ' AND dateDelivered IS NULL AND dateCanceled IS NULL' . ($dateToDeliver ? ' AND dateToDeliver = \'' . $db->cleanDate($dateToDeliver) . '\'' : null))) {
		databaseError($db);
		die('DB error on getWaitingOrders');
	}
	$orderInfo = array ();
	$orders = array ();
...
function getNextDeliveryDay ($day = null, $accountForCutoff = true) {
	...
	if (!$db->query('SELECT * FROM deliveryDay ORDER BY deliveryDayID')) {
		databaseError($db);
		die ('database error on getNextDeliveryDay');
	}
...
function getSchedule ($day = null) {
	...
	if (!$db->query('SELECT deliveryDayID FROM deliveryDay ORDER BY deliveryDayID')) {
		databaseError($db);
		die ('database error on getSchedule');
	}
	$schedule = array ();
	while ($r = $db->getRow(F_RECORD)) {
 ...
 function getDeliveryDays ($idsOnly = false, $dayStart = null, $dayEnd = null) {
	...
	if (!$db->query('SELECT * FROM deliveryDay ORDER BY deliveryDayID')) {
		databaseError($db);
		die ('database error on getDeliveryDays');
	}
	$deliveryDays = array ();
	while ($r = $db->getRow(F_RECORD)) {
		$inSet = false;
...
function getRoutes ($activeOnly = true) {
	...
	if (!$db->query('SELECT * FROM route ' . ($activeOnly ? 'WHERE routeID IN (SELECT routeID FROM routeDay GROUP BY routeID) AND active = 1 ' : null) . 'ORDER BY label')) {
		databaseError($db);
		die ('database error on getRoutes');
	}
	$routes = array ();
	while ($r = $db->getRow(F_RECORD)) {
	...
	if (!$db->query('SELECT * FROM routeDay, deliveryDay WHERE routeDay.deliveryDayID = deliveryDay.deliveryDayID AND routeID IN (' . implode(', ', array_keys($routes)) . ')' . ($activeOnly ? ' AND deliveryDay.active = 1' : null))) {
		databaseError($db);
		die('database error 2 on getRoutes');
	}
	$routeDays = array ();
	while ($r = $db->getRow(F_RECORD)) {
		$routeID = $r->v('routeID');
...
function getPayTypes () {
	...
	if (!$db->query('SELECT * FROM payType ORDER BY label')) {
		databaseError($db);
		die('database error on getPayTypes');
	}
	$payTypes = array ();
	while ($r = $db->getRow(F_RECORD)) {
...
function createPeople ($personIDs) {
	...
	if (!$db->query('SELECT * FROM person WHERE personID IN (' . implode(', ', $personIDs) . ')')) {
		dbError($db);
		die ('database error on createPeople');
	}
	$people = array ();
	while ($r = $db->getRow(F_RECORD)) {
...
abstract class MarketTree extends MarketPrototype {
	...
	protected $_tempSortField;
	public $depth;
	protected $_propsC = array ();
	protected $_numChildren;

	public function getNodePath ($includeThis = true) {
		if (!is_array($this->nodePath)) {
			return null;
		}
	...
	public function hasChildren () {
		if (!is_null($this->_numChildren)) {
			return $this->_numChildren;
		}
		$objectType = $this->getObjectType();
		$objectID = $this->getObjectID();
		global $db;
		if (!$db->query('SELECT * FROM '.$objectType.' WHERE nodePath LIKE "'.$this->getPathString().'/%"')) {
			$this->setError(E_DATABASE, 'on attempt to find children of this node', __CLASS.'::isLeafNode()');
			return false;
		}
		$this->_numChildren = $db->getNumRows();
		return $this->_numChildren;
	}
	...
	public function isInTree () {
		return (bool) (is_array($this->getNodePath()) && count($this->getNodePath()));
	}

	public function getObjectType () {
		return getObjectType($this, true, true);
	}

	public function isSameObjectType ($obj) {
		return (getObjectType($obj) == $this->getObjectType());
	}
	...
	public function getPathString ($includeThis = true) {
		$nodePath = $this->getNodePath($includeThis);
		if (!is_array($nodePath)) {
			return null;
		}
		$pathString = '/'.implode('/', $nodePath);
		return $pathString;
	}
	...
	public function toNodePath ($pathString) {
		...
		$path = explode('/', $pathString); // otherwise, split path string into array
		foreach ($path as $k => $v) {
			$path[$k] = (int) $v;
		}
		return $path;
	...
	public function getChildren ($sortField = null, $directOnly = true, $criteria = null) {
		...
		$objectID = $this->getObjectID();
		if (!$objectID) {
			$this->setError(E_NO_OBJECT_ID, 'no objectID', __CLASS.'::getChildren()');
			return false;
		}
		if (!is_array($criteria)) $criteria = array ();
		global $db;
		$pathString = $this->getPathString(true);
		if (!$db->query('
			SELECT * FROM '.$objectType.' WHERE nodePath '.($directOnly ? ' = "'.$pathString.'/'.$objectID.'"' : 'LIKE "'.$pathString.'/%"'))) {
			$this->setError(E_DATABASE, 'on getting children', __CLASS.'::getChildren()');
			return false;
		}
		$children = array ();
		while ($r = $db->getRow(F_RECORD)) {
		...
		if ($objectType == 'item' && count($children)) {
			if (!$db->query('SELECT * FROM price WHERE itemID IN ('.implode(',', array_keys($children)).')')) {
				$this->setError(E_DATABASE, 'on getting prices for children', __CLASS.'::getChildren()');
				return false;
			}
			$prices = array ();
			while ($r = $db->getRow(F_RECORD)) {
	...
	public function getTree ($sortField = null, $format = 'list', $criteria = null) {
		global $logger;
		if (!is_array($criteria)) $criteria = array ();
		$objectType = $this->getObjectType();
		$objectID = $this->getObjectID();
		if (!$objectID) {
			$this->setError(E_NO_OBJECT_ID, 'No objectID', __CLASS.'::getTree()');
			return false;
		}
		if (!$this->isInTree()) {
			$this->setError(E_INVALID_DATA, 'Object is not in tree', __CLASS.'::getTree()');
			return false;
		}
		$children = $this->getChildren(null, false, $criteria);
		$nodes = array_merge(array($objectID => $this), $children); // include this, the root node
		$this->_tempSortField = $sortField;
		$parent = $this->getParent();
		$tree = $this->treeify($nodes, $this->getParent());
	...
	public function treeify ($flat, $rootNode) {
		global $logger;
		$tree = array ();
		foreach ($flat as $k => $v) {
			if ($v->isChildOf($rootNode)) {
				unset($flat[$k]);
				$thisNode = array ();
				$thisNode['node'] = $v;
				$thisNode['children'] = $this->treeify(&$flat, $v);
	...
	public function getSortFields () {
		if (!$this->_tempSortField) {
			return array (array('dir' => D_ASC, 'field' => null));
		}
		if (is_array($this->_tempSortField)) {
			return $this->_tempSortField;
		}
		$sortFields = explode(',', $this->_tempSortField);
		$sortFields = array_diff($sortFields, array('')); // remove all empty fields
		foreach ($sortFields as $k => $v) {
			...
			$sortFields[$k] = array ('dir' => $sortOrder, 'field' => $v);
		}
		$this->_tempSortField = $sortFields;
		return $sortFields;
	}

	public function getNodeForSort ($node) {
		if (is_array($node)) {
			if (isset($node['node'])) {
				return $node['node'];
			} else {
	...
	public function hasSiblings () {
		...
		if (!$objectID || !$this->isInTree()) {
			$this->setError(E_NO_OBJECT_ID, 'No objectID, or object has no tree sequence', __CLASS.'::hasSiblings()');
			return false;
		}
		if (!$db->query('SELECT '.$objectType.'ID FROM '.$objectType.' WHERE nodePath REGEXP "'.sprintf(DF_NODEPATH_SIBLINGS, $this->getPathString(false)).'"')) {
			$this->setError(E_DATABASE, 'on query for siblings', __CLASS.'::hasSiblings');
			return false;
		}
		return ($db->getNumRows() > 1);
	}

	public function getSiblings ($sortField = null, $includeThis = false) {
		global $db;
		$objectType = $this->getObjectType();
		$objectID = $this->getObjectID();
		if (!$objectID || !$this->isInTree()) {
			$this->setError(E_NO_OBJECT_ID, 'No objectID, or object has no tree sequence', __CLASS.'::getSiblings()');
			return false;
		}
		if (!$db->query('SELECT * FROM '.$objectType.' WHERE nodePath REGEXP "'.sprintf(DF_NODEPATH_SIBLINGS, $this->getPathString(false)).'"'.($includeThis ? null : ' AND '.$objectType.'ID != '.$objectID))) {
			$this->setError(E_DATABASE, 'on query for siblings', __CLASS.'::hasSiblings');
			return false;
		}
		$this->_tempSortField = $sortField;
		$siblings = array ();
		while ($r = $db->fetchRow(F_RECORD)) {
			$siblings[] = $this->newObject($r);
		}
		uasort($siblings, array($this, 'sortBranch'));
		return $siblings;
	}

	public function getSpotInTree ($sortField = null, $rootNodeID = 1, $trimLeafNodes = false) {
		...
		$objectID = $this->getObjectID();
		if (!$objectID || !$this->isInTree()) {
			$this->setError(E_NO_OBJECT_ID, 'No objectID, or object has no tree sequence', __CLASS.'::getSpotInTree()');
			return false;
		}
		$rootNode = $this->newObject((int) $rootNodeID);
		$tree = $rootNode->getTree($sortField, null, array('isLeafNode' => !$trimLeafNodes));
		global $logger;
	...
	public function getPath ($sortOrder = SORT_FORWARD) {
		switch ($sortOrder) {
			...
			case SORT_REVERSE:
				return array_reverse($this->getNodePath());
		}
	}
	...
	public function getToken ($rel = 1) {
		...
		if (is_object($rel)) {
			if ($rel->getObjectType() == $objectType) $rel = $rel->getObjectID();
				...
				$this->setError(E_INVALID_DATA, 'relative object passed is not a proper type of object', __CLASS.'::getDepth()');
				return false;
			}
		}
		$objectID = $this->getObjectID();
		if (!$this->isInTree() || !$objectID) {
			$this->setError(E_NO_OBJECT_ID, 'No objectID (' . $objectID . '), or object has no tree sequence ', __CLASS.'::getToken()');
			return false;
		}
		if ($path = $this->getNodePath()) {
	...
	public function getDepth ($rel = 0) {
		// quick return of cached value
		if (is_int($this->depth) && !$rel) {
			return $this->depth;
		}
		global $logger;
		$objectType = $this->getObjectType();
		$objectID = $this->getObjectID();
		if (!$this->isInTree() || !$objectID) {
			$this->setError(E_NO_OBJECT_ID, 'No objectID (' . $objectID . '), or object has no tree sequence', __CLASS.'::getDepth()');
			return false;
		}
		...
		// if depth hasn't been cached yet, cache it
		if (!is_int($this->depth)) {
			$this->depth = count($this->getNodePath());
		}
		// if $rel is nothing -- zero, false, null, or no objectID, just
		// give this node's depth in relation to node zero
		if (!$rel || (is_object($rel) && !$rel->getObjectID())) {
			return $this->depth;
		}
		if (is_object($rel)) {
			$relID = $rel->getObjectID();
		} else {
			$relID = $rel;
		}
		// if $rel is the same as $this, return a depth of 0
		if ($relID == $objectID) {
			return 0;
		}
		// if $rel is not an object, try to make it an object
		if (!is_object($rel)) {
			$rel = $this->newObject($rel);
		}
		// is $this actually a child node of $rel?
		if (!$this->isIn($rel)) {
			// $this->setError(E_INVALID_DATA, 'This object is not inside the passed object', __CLASS.'::getDepth()');
			return false;
		}
		$depth = $this->depth - $rel->getDepth();
		return $depth;
	}

	public function setParent ($parent, $sortOrder = 0) {
		$objectType = $this->getObjectType();
		$objectID = $this->getObjectID();
		if (!$objectID) {
			$this->setError(E_NO_OBJECT_ID, 'no object ID; has to be saved before you can put it in the tree.', __CLASS.'::setParent()');
			return false;
		}
		if (is_int($parent)) {
			$parent = $this->newObject($parent);
		} else if (!$this->isSameObjectType($parent)) {
			$this->setError(E_INVALID_DATA, 'parent is not the same object type, or is maybe not even an object', __CLASS.'::setParent()');
			return false;
		}
		if ($parent->isIn($this)) {
			$this->setError(E_INVALID_DATA, 'Cannot set self or child to parent', __CLASS.'::setParent()');
			return false;
		}
		global $logger, $db;
		$isInTree = $this->isInTree();
		$oldNodePath = $this->getNodePath();
		$oldPathString = $this->getPathString();
		$oldParentPathString = $this->getPathString(false);
		$newParentNodePath = $parent->getNodePath();
		array_push($newParentNodePath, $objectID);
		$this->nodePath = $newParentNodePath;
		$newPathString = $this->getPathString();
		$newParentPathString = $this->getPathString(false);
		$db->start('setParent'.$objectID);
		$db->startLogging();
		if ($isInTree) {
			if (!$db->query('UPDATE '.$objectType.' SET nodePath = REPLACE(nodePath, "'.$oldPathString.'", "'.$newPathString.'")')) {
				$this->setError(E_DATABASE, 'on changing node paths for current node and children', __CLASS.'::setParent()');
				$db->stopLogging();
				$db->rollback('setParent'.$objectID);
				$this->nodePath = $oldNodePath;
				return false;
			}
			if (!$db->query('UPDATE '.$objectType.' SET sortOrder = sortOrder - 1 WHERE nodePath = "'.$oldParentPathString.'" AND sortOrder > '.(int) $this->sortOrder)) {
				$this->setError(E_DATABASE, 'on shrinking sort order for old siblings', __CLASS.'::setParent()');
				$db->stopLogging();
				$this->nodePath = $oldNodePath;
				$db->rollback('setParent'.$objectID);
				return false;
			...
			if (!$db->query('UPDATE '.$objectType.' SET nodePath = "'.$newPathString.'" WHERE '.$objectType.'ID = '.$objectID)) {
				$this->setError(E_DATABASE, 'on changing node paths for current node', __CLASS.'::setParent()');
				$db->stopLogging();
				$db->rollback('setParent'.$objectID);
				$this->nodePath = $oldNodePath;
				return false;
		...
		if (!$db->query('UPDATE '.$objectType.' SET sortOrder = ((SELECT COALESCE(maxSortOrder, 0) FROM (SELECT MAX(sortOrder) AS maxSortOrder FROM '.$objectType.' WHERE nodePath REGEXP "'.sprintf(DF_NODEPATH_SIBLINGS, $this->getPathString(false)).'" AND '.$objectType.'ID != '.$objectID.') AS maxSortOrderSub) + 1) WHERE '.$objectType.'ID = '.$objectID)) {
			$this->setError(E_DATABASE, 'on setting new sortOrder for this object', __CLASS.'::setParent()');
			$db->stopLogging();
			$db->rollback('setParent'.$objectID);
			$this->nodePath = $oldNodePath;
			return false;
		}
		if (!$db->query('SELECT sortOrder FROM '.$objectType.' WHERE '.$objectType.'ID = '.$objectID)) {
			$this->setError(E_DATABASE, 'on getting new sortOrder for this object', __CLASS.'::setParent()');
			$db->stopLogging();
			$db->rollback('setParent'.$objectID);
			$this->nodePath = $oldNodePath;
			return false;
		}
		if ($sortOrder = $db->getRow()) {
			$logger->log('new sortOrder is', $sortOrder['sortOrder']);
			$this->sortOrder = $sortOrder['sortOrder'];
		}
		$db->stopLogging();
		$db->commit('setParent'.$objectID);
		return true;
	}
	...
	public function isIn ($parent, $includeThis = true) {
		...
		$objectID = $this->getObjectID();
		if (!is_object($parent)) {
			$this->setError(E_INVALID_DATA, '$parent is not an object', __CLASS.'::isIn()');
			return false;
		}
		if (getObjectType($parent, false, true) != getObjectType($this, false, true)) {
			$this->setError(E_INVALID_DATA, '$parent is a ' . get_class($parent) . ' but $this is a ' . get_class($this), __CLASS.'::isIn()');
			return false;
		}
		$parentID = $parent->getObjectID();
		$nodePath = $this->getNodePath($includeThis);
		if (is_array($nodePath)) {
			return in_array($parentID, $this->getNodePath($includeThis));
		} else {
			return false;
		}
	}
	...
	public function contains ($child, $includeThis = true) {
		$objectType = $this->getObjectType();
		if (!is_object($child)) {
			$this->setError(E_INVALID_DATA, '$child is not an object', __CLASS.'::contains()');
			return false;
		}
		if (get_class($child) != get_class($this)) {
			$this->setError(E_INVALID_DATA, '$child is a ' . get_class($child) . ' but $this is a ' . get_class($this), __CLASS.'::contains()');
			return false;
		}
		$objectID = $this->getObjectID();
		return in_array($objectID, $child->getNodePath($includeThis));
	}
	...
	public function getActiveStates () {
		...
		$objectID = $this->getObjectID();
		if (!$objectID) {
			$this->setError(E_NO_OBJECT_ID, 'no objectID', __CLASS.'::getActiveStates()');
			return false;
		}
		$tree = $this->getTree();
		$activeStates = array ();
	...
	private function moveSortOrder ($delta) {
		$objectType = $this->getObjectType();
		$objectID = $this->getObjectID();
		if (!$objectID) {
			$this->setError(E_NO_OBJECT_ID, 'no objectID', __CLASS.'::moveOne()');
			return false;
		}
		if (!$this->isInTree()) {
			$this->setError(E_INVALID_DATA, 'object isn\'t in the tree', __CLASS.'::moveOne()');
			return false;
		}
		if (!$this->hasSiblings()) {
			$this->setError(E_INVALID_DATA, 'object doesn\'t have any siblings', __CLASS.'::moveOne()');
			return false;
		}
		$delta = (int) $delta;
		if (!$delta) {
			$this->setError(E_INVALID_DATA, '$delta is not an integer (or maybe it\'s zero)', __CLASS.'::moveOne()');
			return false;
		}
		$direction = ($delta < 0 ? D_LFT : D_RGT);
		$newSortOrder = $this->sortOrder + $delta;
		if ($newSortOrder < 1) {
			$newSortOrder = 1;
		}
		global $db;
		...
		$db->startLogging();
		if ($direction == D_RGT) {
			if (!$db->query('SELECT MAX(sortOrder) AS maxSortOrder FROM '.$objectType.' WHERE sortOrder >= '.$this->sortOrder.' AND nodePath REGEXP "'.sprintf(DF_NODEPATH_SIBLINGS, $this->getPathString(false)).'"')) {
				$this->setError(E_DATABASE, 'on looking for max sortOrder for this branch', __CLASS.'::moveSortOrder()');
				$db->rollback($t);
				return false;
			}
			if (!($maxSortOrder = $db->getRow(F_ASSOC))) {
				$maxSortOrder = $this->sortOrder;
			} else {
				$maxSortOrder = $maxSortOrder['maxSortOrder'];
			}
			if (!$maxSortOrder) {
				$newSortOrder = $this->sortOrder;
			} else if ($maxSortOrder < $newSortOrder) {
		...
		$newDelta = $newSortOrder - $this->sortOrder;
		$q = 'UPDATE '.$objectType.' SET sortOrder = sortOrder '.($direction == D_LFT ? '+' : '-').' 1 WHERE sortOrder BETWEEN '.($direction == D_LFT ? $newSortOrder : $this->sortOrder).' AND '.($direction == D_LFT ? $this->sortOrder : $newSortOrder).' AND nodePath REGEXP "'.sprintf(DF_NODEPATH_SIBLINGS, $this->getPathString(false)).'"';
		if (!$db->query($q)) {
			$this->setError(E_DATABASE, 'on moving in-between nodes', __CLASS.'::moveOne()');
			$db->rollback($t);
			return false;
		}
		if (!$db->query('UPDATE ' . $objectType . ' SET sortOrder = '.$newSortOrder.' WHERE '.$objectType.'ID = '.$objectID)) {
			$this->setError(E_DATABASE, 'on moving this node', __CLASS.'::moveOne()');
			$db->rollback($t);
			return false;
		}
		$db->stopLogging();
		$db->commit($t);
		global $logger;
		$logger->addEntry('Moved '.$objectType.' '.$this->getLabel().' from '.$this->sortOrder.' to '.$newSortOrder, null, __CLASS.'::moveOne()');
	...
	public function setSortOrder ($sortOrder) {
		if ($this->sortOrder) {
			return $this->moveSortOrder($sortOrder - $this->sortOrder);
		} else {
			return false;
		}
	}

	public function moveOne($direction) {
		if ($direction > 0) {
			$direction = D_RGT;
		} else if ($direction < 0) {
			$direction = D_LFT;
		...
		return $this->moveSortOrder($direction);
	}
	...
	public function getParentID () {
		$path = $this->getNodePath(false);
		global $logger;
		if (is_array($path)) {
			return end($this->getNodePath(false));
		} else {
			return null;
		}
	}
	...
	public function getNeighbor () {
		return call_user_func_array(array($this, 'getNeighbour'), func_get_args());
	}

	public function getNeighbour ($dir = D_LFT, $field = 'lft') {
		$objectType = $this->getObjectType();
		$objectID = $this->getObjectID();
		if (!$objectID) {
			$this->setError(E_NO_OBJECT_ID, 'no objectID', __CLASS.'::getNeighbour()');
			return false;
		}
		if ($objectID == 1) return false;
		if (!$this->lft || !$this->rgt) {
			$this->setError(E_INVALID_DATA, 'object ' . $objectID . ' has no tree sequence', __CLASS.'::getNeighbour()');
			return false;
		}
		global $db;
		switch ($field) {
			case 'lft':
				...
				if (!$db->query('SELECT * FROM ' . $objectType . ' WHERE lft = ' . ($this->$edge + $dir) . ' OR rgt = ' . ($this->$edge + $dir))) {
					$this->setError(E_DATABASE, 'on query', __CLASS.'::getNeighbour()');
					return false;
				}
				if (!$r = $db->getRow(F_RECORD)) {
					$this->setError(E_NO_OBJECT, 'no adjacent object ' . $dir . ' of ' . $objectID, __CLASS.'::getNeighbour()');
					return false;
				}
	...
	public function getNeighborID () {
		return call_user_func_array(array($this, 'getNeighbourID'), func_get_args());
	}
	...
	public function isActive ($includeThis = true) {
		return (bool) $this->getProperty('active', $includeThis, I_PARENT_TRUE);
	}
	...
	*/
	protected function getPropertyList ($property, $includeThis = true, $inherit = I_CHILD) {
		...
		$objectID = $this->getObjectID();
		if (!$objectID) {
			$this->setError(E_NO_OBJECT_ID, 'no objectID', __CLASS.'::getProperty(\'' . $property . '\')');
			return false;
		}
		if (isset($this->_propsC)) {
			if (isset($this->_propsC[$property])) {
				$thisProp = $this->_propsC[$property];
		...
		if (!$db->query('SELECT '.$objectType.'ID, '.$property.' FROM '.$objectType.' WHERE '.$objectType.'ID IN ('.implode(',', $this->getNodePath($includeThis)).') ORDER BY nodePath' . (($inherit == I_CHILD || $inherit == I_CHILD_NULL) ? null : ' DESC'))) {
			$this->setError(E_DATABASE, 'on query', __CLASS.'::getProperty(\'' . $property . '\')');
			return false;
		}
		$propertyList = array ();
		while ($r = $db->getRow(F_RECORD)) {
			...
			if ($v === 't') $v = true;
			if ($v === 'f') $v = false;
			$propertyList[$r->v($objectType.'ID')] = $v;
		}
		if (!isset($this->_propsC)) $this->_propsC = array ();
		$this->_propsC[$property] = array ('includeThis' => (bool) $includeThis, 'inherit' => $inherit, 'v' => $propertyList);
		return $propertyList;
	}
	...
	public function getParent ($parentType = null) {
		...
		if ($objectType == 'person' && (int) $parentType && !array_key_exists($parentType, $GLOBALS['personTypes'])) {
			$this->setError(E_INVALID_DATA, '\'' . (string) $parentType . '\' is not a valid ' . $objectType . ' type', __CLASS.'::getParent()');
			return false;
		}
		if ($objectID == 1) return $this->newObject();
		if (!(int) $parentType) {
			$parent = $this->newObject($this->getParentID());
			return $parent;
		}
		global $db;
		if (!$db->query('SELECT * FROM ' . $objectType . ' WHERE '.$objectType.'ID IN ('.implode(',', $this->getNodePath(false)).')'.((int) $parentType ? ' AND ('.$objectType.'Type & '.(int) $parentType.' > 0)' : null).' ORDER BY nodePath DESC LIMIT 0,1', true)) {
...
class Logger {
		...
		foreach ($args as $arg) {
			switch (gettype($arg)) {
				...
				case 'object':
					...
					break;
				default:
					$logString .= 'type '.gettype($arg);
			}
			$logString .= ' ';
		}
		$this->addEntry($logString);
	}
	...
	public function addEntry ($entryText, $errorCode = 0, $source = null) {
		...
		$stateStack = implode(' > ', $this->stateStack);
		if ($stateStack) {
			$stateStack .= ' > ';
		}
		...
		switch ($this->logType) {
			case LOG_FILE:
				...
				fwrite($this->logFile, '[' . strftime('%Y/%m/%d %H:%M:%S') . '] ' . basename($_SERVER['SCRIPT_FILENAME']) . ': ' . $stateStack . ($source ? $source . ': ' : null) . ($errorCode ? $errorCodes[$errorCode] . ' - ' : null) . (is_array($entryText) || is_object($entryText) ? print_r($entryText, true) : $entryText) . "\n");
				return true;
			case LOG_OUTPUT:
				if (!$GLOBALS['ajax']) {
			...
			case LOG_DB:
				...
				$q = 'INSERT INTO logEntry (dateCreated, errorCode, page, source, requestVars, sessionVars, entryText, personID) VALUES (NOW(), ' . (int) $errorCode . ', \'' . $db->cleanString($_SERVER['PHP_SELF']) . '\', ' . ($source ? '\'' . $db->cleanString($source) . '\'' : 'null') . ', \'' . $db->cleanString(serialize($_REQUEST)) . '\', \'' . $db->cleanString(serialize($_SESSION)) . '\', \'' . $db->cleanString($stateStack . $entryText) . '\', ' . ($personID ? (int) $personID : 'null') . ')';
				if ($db->getKey()) $this->buffer[] = $q;
				else if (count($this->buffer)) $this->flushBuffer();
				$db->query($q, false, false);
				return true;
			case LOG_CONSOLE:
				$this->buffer[] = '[' . strftime('%Y/%m/%d %H:%M:%S') . '] ' . basename($_SERVER['SCRIPT_FILENAME']) . ': ' . $stateStack . ($source ? $source . ': ' : null) . ($errorCode ? $errorCodes[$errorCode] . ' - ' : null) . (is_array($entryText) || is_object($entryText) ? print_r($entryText, true) : $entryText) . "\n";
				return true;
			case LOG_NONE:
			default:
				return false;
	...
	public function push ($val) {
		$val = (string) $val;
		if ($val) {
			array_push($this->stateStack, $val);
			return true;
		}
		return false;
	}
	...
	public function pop ($val = null) {
		...
		if (!$val || ($val == end($this->stateStack))) {
